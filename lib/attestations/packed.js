 ...
 * 
 * Slightly modified version of validateSignature
 */
function validateSurrogateSignature(rawClientData, authenticatorData, sig, hashAlg, pemKey) {
    // create clientDataHash
    const hash = crypto.createHash("sha256");
    hash.update(abToBuf(rawClientData));
    var clientDataHashBuf = hash.digest();
    ...
    return verify.verify({key: pemKey}, abToBuf(sig))
}
...
/**
 * Self attestation (i.e. surrogate basic attestation) verification
 * 
 * Numbered steps from https://w3c.github.io/webauthn/#packed-attestation
 */
async function packedValidateSurrogate() {
    ...
    if (algName !== credentialPublicKeyJwk.alg) {
        throw new Error("self attestation algorithm mismatch");
    }
    // 4.2. Verify that sig is a valid signature over the concatenation of authenticatorData and clientDataHash
    //      using the attestation public key in x5c with the algorithm specified in alg.
    var res = validateSurrogateSignature(
        this.clientData.get("rawClientDataJson"),
        this.authnrData.get("rawAuthnrData"),
        this.authnrData.get("sig"),
        hashAlg,
    ...
    this.audit.journal.add("alg");
    this.audit.journal.add("sig");
    // 4.3. If successful, return implementation-specific values representing attestation type Self and an empty attestation trust path.
    this.audit.info.set("attestation-type", "self");
    // x5c not applicable, so let's consider audited
